...
1. **템플릿 기반 시작, 차이만 각색** — 스택이 같으면(바닐라 JS + Node 프록시) 파일을
   ...
   스택상 불가피하지 않다면 `api/map/panel/chart/format/proxy` 모듈 경계를 유지하고,
...
6. **캐시 정책** — 원본은 검색은 no-cache, 뷰포트 마커는 60초 TTL
   (`MARKER_CACHE_TTL_MS` — 유형 필터 왕복 등 동일 뷰 재조회의 크레딧 재과금 방지,
   0이면 캐시 없음), 단지 단위 데이터는 세션 캐시로 분화돼 있다. 가격 신선도가
   중요한 서비스는 TTL 또는 수동 갱신으로 조정한다.
7. **호출 비용** — 주변 단지 비교는 패널을 열 때 1+6콜을 자동 실행한다. 크레딧이
   민감한 서비스는 섹션 진입 시 로드(lazy)로 전환할 수 있다.
...
js/panel.js       단지 패널 (게이지·실거래·주변 비교·평형/동/호 드릴다운·입지·개요)
