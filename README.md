...
| **지역 아파트 순위** (`region-apartment-ranking`) | "강남구 아파트 순위"처럼 물으면, 한 지역(구·동)의 아파트를 **세대수·연식·역세권·실거래가 중 원하는 기준으로 순위**를 매겨 목록 카드(HTML)로 보여줍니다. | 기획자·마케터·중개사 등 누구나 |
...
![아파트 가격 진단 데모](docs/price-check-v2.gif)
...
→ 한 지역의 아파트를 세대수·연식·역세권·실거래가 기준으로 순위 매긴 목록 카드가 HTML 파일로 만들어집니다.

![지역 아파트 순위 데모](docs/region-ranking-v2.gif)
